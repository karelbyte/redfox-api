...
import { ImportLogType, ImportLogStatus } from '../../src/models/import-log.entity';
...
const mockClientImportService = {
  importRows: jest.fn().mockResolvedValue({
    created: 5, skipped: 1, pack_synced: 2, pack_failed: 0,
    errors: [], pack_warnings: [],
...
describe('ImportProcessor', () => {
  ...
  it('registra el processor en InMemoryQueue al iniciar', () => {
    ...
    expect(mockInMemoryQueue.registerProcessor).toHaveBeenCalledWith(expect.any(Function));
  });

  describe('job type: client', () => {
    const job = { ...baseJob, type: 'client' as const, rows: [{ row: 1, code: 'C1', name: 'Test' }] };

    it('crea log pending al inicio', async () => {
      await processor.process(job);
      expect(mockImportLogService.createPending).toHaveBeenCalledWith(
        ImportLogType.CLIENT, 'user-1', 'org-1', 1,
      );
    });

    it('llama a clientImportService.importRows con el orgId correcto', async () => {
      await processor.process(job);
      expect(mockClientImportService.importRows).toHaveBeenCalledWith(job.rows, 'org-1');
    });

    it('completa el log con los resultados', async () => {
      await processor.process(job);
      expect(mockImportLogService.complete).toHaveBeenCalledWith('log-1', expect.objectContaining({
        created_count: 5,
        skipped_count: 1,
        error_count: 0,
      }));
  ...
  describe('job type: product', () => {
    const job = { ...baseJob, type: 'product' as const, rows: [{ row: 1, name: 'P1', sku: 'SKU1', code: '12345678', measurement_unit: 'H87' }] };

    it('llama a productImportService.importRowsWithOrg', async () => {
      await processor.process(job);
      expect(mockProductImportService.importRowsWithOrg).toHaveBeenCalledWith(job.rows, 'org-1');
    });
  ...
  describe('job type: provider', () => {
    const job = { ...baseJob, type: 'provider' as const, rows: [{ row: 1, code: 'P1', name: 'Proveedor' }] };

    it('llama a providerImportService.importRows', async () => {
      await processor.process(job);
      expect(mockProviderImportService.importRows).toHaveBeenCalledWith(job.rows, 'org-1');
    });
  ...
  describe('manejo de errores', () => {
    it('marca el log como failed si el servicio lanza error', async () => {
      mockClientImportService.importRows.mockRejectedValueOnce(new Error('DB connection lost'));
      const job = { ...baseJob, type: 'client' as const, rows: [] };
      await processor.process(job);
      expect(mockImportLogService.fail).toHaveBeenCalledWith('log-1', expect.stringContaining('DB connection lost'));
    });

    it('crea notificación de error si falla', async () => {
      mockClientImportService.importRows.mockRejectedValueOnce(new Error('Timeout'));
      const job = { ...baseJob, type: 'client' as const, rows: [] };
      await processor.process(job);
      expect(mockNotificationRepo.save).toHaveBeenCalled();
    });
