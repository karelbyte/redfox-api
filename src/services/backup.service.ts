...
export class BackupService implements OnModuleInit {
    ...
    private scheduleBackup(config: BackupConfig) {
        ...
        job.start();
        this.logger.log(`Backup scheduled with frequency: ${config.frequency} at ${config.scheduledTime}`);
    }
    ...
    async runBackup(triggerType: string = 'manual'): Promise<BackupLog> {
        ...
        try {
            ...
            try {
                const checkCmd = isPostgres ? `${dumpTool} --version` : `${dumpTool} --version`;
                await execPromise(checkCmd);
            } catch (e) {
                const installCmd = isPostgres ? 'brew install libpq' : 'brew install mysql-client';
                throw new Error(
