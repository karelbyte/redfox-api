...
export class ReferralService {
  ...
  async getReferrers(page = 1, limit = 20, search?: string) {
    ...
    if (search) {
      qb.where(
        'LOWER(r.name) LIKE :s OR LOWER(r.code) LIKE :s OR LOWER(r.email) LIKE :s',
        {
  ...
  async getReferrer(id: string) {
    const r = await this.referrerRepo.findOne({
      where: { id },
      relations: ['commissions', 'commissions.organization'],
    });
  ...
  async deleteReferrer(id: string) {
    const r = await this.referrerRepo.findOne({ where: { id } });
    if (!r) throw new NotFoundException('Referente no encontrado');
    const hasCommissions = await this.commissionRepo.count({
      where: { referrer_id: id },
    });
    if (hasCommissions > 0)
      throw new BadRequestException(
        'No se puede eliminar un referente con comisiones registradas',
      );
  ...
  async validateCode(code: string) {
    ...
    return r
      ? { valid: true, referrer: { id: r.id, name: r.name, code: r.code } }
      : { valid: false };
  }
  ...
  async getStats() {
    ...
    const pending = await this.commissionRepo.count({
      where: { status: 'pending' },
    });
    const approved = await this.commissionRepo.count({
